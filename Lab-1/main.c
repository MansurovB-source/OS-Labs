...
uint64_t thread_init_for_read(void) {
	...
	for(uint8_t i = 0; i < READ_THREAD_NUM - 1; i+= 2) {
		struct thread_args* args_1 = (struct thread_args*) malloc (sizeof (struct thread_args));
		struct thread_args* args_2 = (struct thread_args*) malloc (sizeof (struct thread_args));
